...
class Store(object):
    ...
    def build_config_from_db(self, transaction_id):
        ...
        if self.Transactions[transaction_id].ClientConfigDB == None:
            query = "SELECT hostname, mac, NULL as duid, 'default' as class, address, NULL as id FROM %s WHERE mac = '%s' AND vlan = '%s'" % \
                    (self.table_hosts,\
                     self.Transactions[transaction_id].MAC,\
                     re.sub("[0-9]+[:\.]|\D","",self.Transactions[transaction_id].Interface))
            answer = self.query(query)
...
class PGSQL(Store):
    ...
    def DBConnect(self):
        ...
        try:
            self.connection = psycopg2.connect(host=self.cfg.STORE_PGSQL_HOST,\
                                               database=self.cfg.STORE_PGSQL_DB,\
                                               user=self.cfg.STORE_PGSQL_USER,\
                                               password=self.cfg.STORE_PGSQL_PASSWORD)
